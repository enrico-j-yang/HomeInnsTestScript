...
class KoodarIOSAssistantButtonAtomTests(unittest.TestCase):
    ...
    # in ios simulator, this case will fail due to the poor performance
    def button_launch(self):
        self.__common_launch_app()
        self.__common_login()

        # enter my account 
        self.testStep.wait_widget("//UIACollectionCell[@name='虚拟旋钮']")
        #click into Navigation
        self.testStep.tap_widget("//UIACollectionCell[@name='虚拟旋钮']")
        
        # wait for my account activity
        #self.testStep.wait_widget("//UIAStaticText[@name='虚拟旋钮']")
        self.testStep.driver.switch_to_alert().accept()
        self.testStep.wait_widget("//UIAStaticText[@name='指引页']")

        # enter account info
        self.testStep.tap_widget("//UIAButton[@name='Back']")
        self.testStep.tap_widget("//UIAButton[@name='Back']")
    ...
    atom_test_list = {'button_launch':button_launch
                        }
            
class KoodarIOSAssistantButtonTests(unittest.TestCase):
    think_time = 3
    ...
    def setUp(self):
        ...
        self.atomTest = KoodarIOSAssistantButtonAtomTests(self.testStep, desired_caps['platformName'])
    ...
    def test_button_launch(self):
        self.atomTest.button_launch()
