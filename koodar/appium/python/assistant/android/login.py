...
class KoodarAndroidAssistantLoginTests(unittest.TestCase):
    ...
    def _common_logout(self):      
        ...
        if self.testStep.driver.wait_activity('systems.xos.car.android.product.companion.MainActivity', 5, 1):
            # enter my account 
            self.testStep.tap_widget("//android.widget.LinearLayout[1]/android.widget.FrameLayout[1]/android.widget.LinearLayout[1]\
            ...
            # enter account info
            self.testStep.tap_widget("//android.widget.TextView[@text='账户信息']")
            ...
            # log out
            self.testStep.tap_widget("//android.widget.TextView[@text='退出登录']")
            # confirm log out
            self.testStep.tap_button("android:id/button1")
        else:
