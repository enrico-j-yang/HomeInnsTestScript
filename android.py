...
class KoodarAssistantAndroidTests(unittest.TestCase):
    ...
    def __common_login(self):
        ...
        if self.driver.wait_activity('systems.xos.car.android.product.companion.startup.login.LoginActivity', 3, 1):
            ...
            phone_number.send_keys("18039499677")
...
#@unittest.skip("demostrating skipping")
    def test_login(self):
        ...
        try:
            ...
            phone_number.send_keys(u"18039499677")
            password = self.driver.find_element_by_id('com.gexne.car.assistant:id/login_password')
            self.assertTrue(password)
            self.touchAction.press(password).release().perform() # because send_keys miss first character, so here come one blank as to avoid this problem
            password.send_keys(u"ASDFGHJKL")
            sleep(self.__think_time)
            login = self.driver.find_element_by_id('com.gexne.car.assistant:id/login_next')
            self.assertTrue(login)
            self.touchAction.press(login).release().perform()
    ...
    def test_navigation(self):
        ...
        sleep(self.__think_time)
        get_route_layout = self.driver.find_element_by_id("com.gexne.car.assistant:id/get_route_layout")
        self.assertTrue(get_route_layout)
        self.touchAction.press(get_route_layout).release().perform()
        sleep(self.__think_time)
        push_destination = self.driver.find_element_by_id("com.gexne.car.assistant:id/push_destination")
        self.assertTrue(push_destination)
        self.touchAction.press(push_destination).release().perform()
        sleep(self.__think_time)
        back_btn = self.driver.find_element_by_id("com.gexne.car.assistant:id/back_btn")
        self.assertTrue(back_btn)
        self.touchAction.press(back_btn).release().perform()
        sleep(self.__think_time)
        push_layout = self.driver.find_element_by_id("com.gexne.car.assistant:id/push_layout")
        self.assertTrue(push_layout)
        self.touchAction.press(push_layout).release().perform()
        sleep(self.__think_time)
        star_btn = self.driver.find_element_by_id("com.gexne.car.assistant:id/star_btn")
        self.assertTrue(star_btn)
        self.touchAction.press(star_btn).release().perform()
        sleep(self.__think_time)
        favorite_btn = self.driver.find_element_by_id("com.gexne.car.assistant:id/favorite_btn")
        self.assertTrue(favorite_btn)
        self.touchAction.press(favorite_btn).release().perform()
        sleep(self.__think_time)
